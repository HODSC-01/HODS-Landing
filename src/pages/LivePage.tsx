import { Link } from 'react-router-dom'
import { useState } from 'react'
import welcomeImg from '../assets/welcometohod.png'

const YT_CHANNEL_HANDLE = 'houseofdayspring8395'
const YT_LIVE_SRC = `https://www.youtube.com/embed/live_stream?channel=${YT_CHANNEL_HANDLE}&autoplay=0&rel=0&modestbranding=1`
const FB_LIVE_PAGE_URL = 'https://www.facebook.com/HODLiveFeed'
const FB_EMBED_SRC = `https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(FB_LIVE_PAGE_URL + '/videos')}&show_text=0&mute=0&autoplay=false&width=800`

type Tab = 'youtube' | 'facebook'

export default function LivePage() {
  const [activeTab, setActiveTab] = useState<Tab>('youtube')

  return (
    <div className="min-h-screen bg-[#080e20] text-white font-sans antialiased flex flex-col">

      {/* ── Top Bar ── */}
      <header className="w-full border-b border-white/10 bg-[#080e20]/95 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 h-16 flex items-center justify-between">
          {/* Back to Home */}
          <Link
            to="/"
            className="inline-flex items-center gap-2 text-sm text-white/60 hover:text-white transition-colors font-medium group"
          >
            <svg
              className="w-4 h-4 transition-transform group-hover:-translate-x-0.5"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
            >
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
            </svg>
            Back to site
          </Link>

          {/* Brand */}
          <Link to="/" className="flex items-center gap-2.5">
            <img src={welcomeImg} alt="HODi logo" className="w-8 h-8 object-contain" />
            <span className="font-bold text-sm sm:text-base tracking-tight text-white whitespace-nowrap">
              House of <span className="text-[#c9a030]">Dayspring</span>
            </span>
          </Link>

          {/* Live indicator */}
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-600/20 border border-red-500/30 text-red-400 text-xs font-bold tracking-widest uppercase">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-500 opacity-75" />
              <span className="relative inline-flex rounded-full h-2 w-2 bg-red-500" />
            </span>
            Live
          </div>
        </div>
      </header>

      {/* ── Gold accent line ── */}
      <div className="w-full h-0.5 bg-gradient-to-r from-transparent via-[#c9a030] to-transparent" />

      {/* ── Main Content ── */}
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-12 py-8 sm:py-12 flex flex-col">

        {/* Section header */}
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#c9a030] mb-2">
              House of Dayspring International Church
            </p>
            <h1 className="text-[clamp(2rem,5vw,3.5rem)] font-black text-white leading-tight tracking-tight">
              Watch <span className="text-[#c9a030]">HODi</span> Live
            </h1>
            <p className="text-sm text-white/55 mt-2 max-w-md leading-relaxed">
              Join us live for Sunday worship, special services, and powerful messages — streamed directly to you, wherever you are.
            </p>
          </div>

          {/* Platform Tab Switcher */}
          <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl p-1 self-start sm:self-auto">
            <button
              type="button"
              onClick={() => setActiveTab('youtube')}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-xs font-bold tracking-wide transition-all duration-200 ${
                activeTab === 'youtube'
                  ? 'bg-[#FF0000] text-white shadow-lg shadow-red-900/30'
                  : 'text-white/60 hover:text-white hover:bg-white/5'
              }`}
            >
              <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
              </svg>
              YouTube Live
            </button>
            <button
              type="button"
              onClick={() => setActiveTab('facebook')}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-xs font-bold tracking-wide transition-all duration-200 ${
                activeTab === 'facebook'
                  ? 'bg-[#1877F2] text-white shadow-lg shadow-blue-900/30'
                  : 'text-white/60 hover:text-white hover:bg-white/5'
              }`}
            >
              <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
              </svg>
              Facebook Live
            </button>
          </div>
        </div>

        {/* ── Player ── */}
        <div className="relative w-full flex-1 rounded-2xl overflow-hidden bg-black border border-white/10 shadow-[0_0_80px_rgba(201,160,48,0.08)]">
          <div className="absolute inset-0 pointer-events-none rounded-2xl ring-1 ring-white/5 z-10" />

          {activeTab === 'youtube' && (
            <div className="w-full aspect-video">
              <iframe
                key="yt-live"
                className="w-full h-full border-0"
                src={YT_LIVE_SRC}
                title="House of Dayspring International Church — YouTube Live"
                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                allowFullScreen
              />
            </div>
          )}

          {activeTab === 'facebook' && (
            <div className="w-full flex flex-col">
              <div className="bg-[#1877F2]/15 border-b border-[#1877F2]/25 px-5 py-2.5 flex flex-wrap items-center gap-2 text-xs text-blue-300">
                <svg className="w-3.5 h-3.5 fill-current text-[#1877F2] shrink-0" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
                <span>If the stream doesn't load, allow third-party cookies in your browser settings, or</span>
                <a
                  href={FB_LIVE_PAGE_URL}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="underline font-bold text-white hover:text-blue-200"
                >
                  open directly on Facebook →
                </a>
              </div>
              <div className="w-full aspect-video">
                <iframe
                  key="fb-live"
                  className="w-full h-full border-0"
                  src={FB_EMBED_SRC}
                  title="House of Dayspring International Church — Facebook Live"
                  allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share"
                  allowFullScreen
                  scrolling="no"
                />
              </div>
            </div>
          )}
        </div>

        {/* ── Footer row ── */}
        <div className="mt-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <p className="text-xs text-white/35 leading-relaxed">
            Sundays 8:00 AM – 11:00 AM • House Fellowship Mondays • Bible Study Thursdays 5 PM<br />
            House of Dayspring International Church — The Pacesetters Assembly
          </p>
          <div className="flex flex-wrap gap-3">
            <a
              href={`https://www.youtube.com/@${YT_CHANNEL_HANDLE}?sub_confirmation=1`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#FF0000] hover:bg-red-700 text-white font-bold text-xs shadow-md transition-all hover:scale-105"
            >
              <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
                <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
              </svg>
              Subscribe on YouTube
            </a>
            <a
              href={FB_LIVE_PAGE_URL}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#1877F2] hover:bg-blue-700 text-white font-bold text-xs shadow-md transition-all hover:scale-105"
            >
              <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
              </svg>
              Open Facebook Live
            </a>
            <a
              href="https://t.me/hodmessages"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#229ED9] hover:bg-sky-600 text-white font-bold text-xs shadow-md transition-all hover:scale-105"
            >
              <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
                <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69a.2.2 0 00-.05-.18c-.06-.05-.14-.03-.21-.02-.09.02-1.49.95-4.22 2.79-.4.27-.76.41-1.08.4-.36-.01-1.04-.2-1.55-.37-.63-.2-1.12-.31-1.08-.66.02-.18.27-.37.74-.56 2.91-1.27 4.86-2.11 5.83-2.52 2.78-1.16 3.35-1.36 3.73-1.36.08 0 .27.02.39.12.1.08.13.19.14.27-.01.06.01.24 0 .38z"/>
              </svg>
              Telegram
            </a>
          </div>
        </div>
      </main>
    </div>
  )
}
